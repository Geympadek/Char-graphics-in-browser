...
function edgeFunction(a: {x: number, y: number}, b: {x: number, y: number}, p: {x: number, y: number})
{
...
function fillWith(filler : string)
{
    screenOutput = "";
...
function symbolFromBrightness(brightness : number)
{
...
function brightnessFromPixels(symbolx : number, symboly : number)
{
    let brightness = 0;
